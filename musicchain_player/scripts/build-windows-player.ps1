...
<#
.SYNOPSIS
  Build the musicchain Windows player (Flutter desktop).

.DESCRIPTION
  Required env vars (no defaults):
    FLUTTER     Path to flutter.bat / flutter binary
    MC_DLL_DIR  Where to find a freshly-built musicchain.dll +
...
if (-not $MC_DLL_DIR) { Write-Error "MC_DLL_DIR not set — point at a folder holding musicchain.dll + mc_rats.dll" }
