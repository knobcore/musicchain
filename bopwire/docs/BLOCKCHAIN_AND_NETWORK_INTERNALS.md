# Bopwire — Blockchain & Network Stack Internals

> **Scope.** This document describes, step by step, how the Bopwire blockchain
...
Bopwire is an **account-based** (not UTXO) blockchain whose primary on-chain asset
...
|---|---|---|---|
| **Full node** | `bopwire-node` | Yes — `mc::Chain` + `BlockPropagator` + `CandidateManager` | Content-addressed file store |
| **Mini-node** | `bopwire-mini-node` | No — pure relay/router | No |
...
inside `bopwire-node`. The transport (section 13) is **librats**, a vendored QUIC/TCP
...
Bopwire tx must SHA-256 the preimage, not keccak it.
...
This means a Bopwire signature can never replay on Ethereum/BSC/Base (their chain_id
...
`BOPWIRE_VPS_BOOTSTRAP`, defaulting to `127.0.0.1:8080` then the hardcoded VPS
...
1. **RPC** — `bopwire.request` / `bopwire.reply`. Envelope
...
3. **Routes** — `bopwire.routes` topic. `build_route_json` ([`rats_link.cpp:148-172`](../src/network/rats_link.cpp#L148))
...
catch-up. Every verb rides a `bopwire.request` envelope with `type` beginning `block.`,
