...
// bopwire runs a PRIVATE DHT overlay on top of the KRPC wire format. Every
...
std::unique_ptr<KrpcMessage> KrpcProtocol::decode_message(const std::vector<uint8_t>& data) {
    try {
        ...
        // This is the core of the bopwire DHT isolation — packets from the
        // public BitTorrent mainline (or any non-bopwire peer) are silently
        ...
        if (!root.has_key(KRPC_NETWORK_KEY) ||
            ...
            LOG_KRPC_DEBUG("Dropping non-bopwire KRPC packet (missing/invalid network tag)");
            return nullptr;
        }
